<template>
  <div>
    ...
    <div>
      <mu-row gutter>
         <mu-raised-button :label="'当天人次：'+receptionList.length" class="demo-raised-button" fullWidth primary backgroundColor="#00c853" @click="fetchAllReceptionList"/>
      </mu-row>
    <mu-row>
      <mu-col>
      </mu-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchReceptionList () {
        this.$http.get('/api/reception/list.do', {
          params: {
      ...
      fetchAllReceptionList () {
        this.$http.get('/api/manage/reception/list.do', {
          params: {
            date: new Date().getTime(),
            pageNum: 1,
            pageSize: 100
